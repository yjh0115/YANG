...
if(isMission){const result=document.getElementById('result'),grade=document.getElementById('grade'),retry=document.getElementById('retry');if(result&&!document.getElementById('missionNextStep')){const next=document.createElement('div');next.id='missionNextStep';result.insertAdjacentElement('afterend',next);grade?.addEventListener('click',()=>{next.replaceChildren();const p=document.createElement('p');p.textContent='수고했어요! 해설을 확인하고 복습하거나 학습 목록으로 돌아가세요.';next.append(p,makeLink('🧩 관계 지도 복습','relation-map.html'),makeLink('🏠 학습 목록으로','index.html'))});retry?.addEventListener('click',()=>next.replaceChildren())}}
if(isHome){
