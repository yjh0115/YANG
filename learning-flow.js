...
if(isMission){const result=document.getElementById('result'),grade=document.getElementById('grade'),retry=document.getElementById('retry'),form=document.getElementById('mission');if(result&&!document.getElementById('missionNextStep')){const next=document.createElement('div');next.id='missionNextStep';result.insertAdjacentElement('afterend',next);grade?.addEventListener('click',()=>{next.replaceChildren();const p=document.createElement('p');p.textContent='수고했어요! 해설을 확인하고 복습하거나 학습 목록으로 돌아가세요.';next.append(p,makeLink('🧩 관계 지도 복습','relation-map.html'),makeLink('🏠 학습 목록으로','index.html'))});retry?.addEventListener('click',()=>next.replaceChildren());form?.addEventListener('change',()=>{if(result.hidden)return;result.hidden=true;result.textContent='';form.querySelectorAll('.question').forEach(q=>q.classList.remove('checked','correct','wrong'));next.replaceChildren();const notice=document.createElement('p');notice.textContent='답을 변경했어요. 현재 답안으로 다시 채점해 주세요.';next.append(notice)})}}
if(isHome){
