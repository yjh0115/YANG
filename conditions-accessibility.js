...
renderConditions=function(area){
 ...
 let selected=null;
 const update=()=>{const count=cards.filter(card=>card.parentElement===zone).length;note.textContent=`올바른 조건 ${count}/3개 분류 완료. ${selected?'선택한 조건: '+selected.textContent:'카드를 선택해 주세요.'}`;};
 ...
 const choose=card=>{selected=card;cards.forEach(item=>{const active=item===card;item.setAttribute('aria-pressed',String(active));item.style.outline=active?'3px solid #7652ca':'';});add.disabled=false;update();};
 ...
 add.addEventListener('click',()=>{if(!selected)return;const card=selected,ok=card.dataset.c!=='1';feedback.textContent=ok?'맞아요. 이 조건은 평행사변형을 보장해요.':'대각선의 길이가 같은 등변사다리꼴도 있어요. 반드시 평행사변형은 아니에요.';feedback.className=ok?'feedback good':'feedback bad';if(ok)zone.append(card);clear();const next=cards.find(item=>item.parentElement!==zone);(next||add).focus();});
