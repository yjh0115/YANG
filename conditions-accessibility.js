...
renderConditions=function(area){
 ...
 cards.forEach(card=>{card.setAttribute('role','button');card.tabIndex=0;card.setAttribute('aria-pressed','false');card.setAttribute('aria-label',`조건 카드: ${card.textContent}`);if(instruction)card.setAttribute('aria-describedby',instruction.id);card.addEventListener('click',()=>choose(card));card.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();choose(card);}});card.addEventListener('dragstart',()=>{dragged=card.parentElement!==zone&&!complete()?card:null;});card.addEventListener('dragend',()=>{dragged=null;});});
