...
renderConditions=function(area){
 ...
 const update=()=>{const count=cards.length-remaining().length;note.textContent=`올바른 조건 ${count}/3개 분류 완료. ${count===3?'모든 올바른 조건을 찾았어요.':selected?'선택한 조건: '+selected.textContent:'카드를 선택해 주세요.'}`;};
 ...
 const choose=card=>{if(card.parentElement===zone)return;selected=card;cards.forEach(item=>{const active=item===card;item.setAttribute('aria-pressed',String(active));item.style.outline=active?'3px solid #7652ca':'';});add.disabled=false;update();};
 ...
 add.addEventListener('click',()=>{if(!selected)return;const card=selected,ok=card.dataset.c!=='1';feedback.textContent=ok?'맞아요. 이 조건은 평행사변형을 보장해요.':'대각선의 길이가 같은 등변사다리꼴도 있어요. 반드시 평행사변형은 아니에요.';feedback.className=ok?'feedback good':'feedback bad';if(ok){zone.append(card);card.tabIndex=-1;card.setAttribute('role','listitem');card.removeAttribute('aria-pressed');}clear();const next=remaining().find(item=>item.dataset.c!=='1')||remaining()[0];const target=next||(area.closest('#lessonView')?.querySelector('#nextStep'));if(target)target.focus();});
 zone.addEventListener('drop',()=>{cards.filter(card=>card.parentElement===zone).forEach(card=>{card.tabIndex=-1;card.setAttribute('role','listitem');card.removeAttribute('aria-pressed');});clear();});
 update();
};
})();