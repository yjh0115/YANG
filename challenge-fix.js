...
renderQuiz=function(area,lesson,challenge){if(!challenge){original(area,lesson,false);return;}const lessonId=lesson.id;const task=tasks[current];const previouslyAttempted=hasAttempt(lessonId);if(previouslyAttempted)area.dataset.challengeAttempted='yes';else delete area.dataset.challengeAttempted;area.innerHTML=`<div class="practice-card"><span class="eyebrow dark">CHALLENGE · 생각 넓히기</span><h3>${task.q}</h3><div class="quiz-options">${task.opts.map((option,i)=>`<button type="button" data-reason-answer="${i}">${i+1}. ${option}</button>`).join('')}</div><div id="quizFeedback" role="status" aria-live="polite"></div><div class="hint-row"><button type="button" class="hint-btn" id="reasonHint">힌트 보기</button></div></div>`;if(isCompleted(lessonId))area.querySelector('#quizFeedback').textContent='이미 완료한 탐구예요. 다시 풀거나 학습 완료 화면으로 이동할 수 있어요.';else if(previouslyAttempted)area.querySelector('#quizFeedback').textContent='이전에 답을 선택한 기록이 있어요. 다시 풀거나 채점하고 마치기를 눌러도 됩니다.';area.querySelectorAll('[data-reason-answer]').forEach(button=>button.onclick=()=>{recordAttempt(lessonId);area.dataset.challengeAttempted='yes';const ok=Number(button.dataset.reasonAnswer)===task.answer;area.querySelector('#quizFeedback').innerHTML=`<div class="feedback ${ok?'good':'bad'}">${ok?'정답이에요! '+task.why:'조건을 모두 사용했는지 살펴보세요. 힌트를 참고하고 다시 도전해요.'}</div>`;});area.querySelector('#reasonHint').onclick=()=>{area.querySelector('#quizFeedback').innerHTML='<div class="feedback bad">기본 성질을 하나씩 적용하고, 주어지지 않은 조건을 마음대로 가정하지 않았는지 확인해 보세요.</div>';};};
